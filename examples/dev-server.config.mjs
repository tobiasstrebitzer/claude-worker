/**
 * The dev gateway, as a `claude-worker` config file. Edit it directly — it is a
 ...
 *
 * There is no separate dev entry point any more: this runs the same `claude-worker`
 * binary a user gets from `npx claude-worker`, so the thing you develop against and
 ...
 *
 * Parked sessions land in `examples/.claude-worker/` (gitignored) and survive a
 ...
 * one — printed once, kept in the state dir (`examples/.claude-worker/auth-key`
...
export default {
   ...
   * from `~/.claude`, exactly like plain `npx claude-worker`, and sessions run
