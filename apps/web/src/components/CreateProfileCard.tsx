...
export function CreateProfileCard({ onCreated }: { onCreated: () => Promise<void> | void }) {
  ...
  const submit = async () => {
    ...
    if (isProvider) {
      ...
      if (capabilities.length > 0) session.capabilities = capabilities
      if (commaList(mcpServers).length > 0) session.mcpServers = commaList(mcpServers)
        ...
        {isProvider ? (
          <>
            ...
            <p className='text-label text-fg-4'>
              Leave capabilities and MCP servers empty to inherit whatever the server&apos;s engine
              factory wired — that factory is the ceiling either way, so a grant here can never
              exceed it. MCP servers are named, never configured: their transport config (and any
              credentials in it) stays on the server.
            </p>
          </>
        ) : (
          <>
            ...
            <p className='text-label text-fg-4'>
              Must resolve inside the server&apos;s{' '}
              <code className='font-mono'>allowedConfigDirRoots</code> — a config directory is a
